...
class RegisterViewController: UIViewController {
    ...
    @IBAction func guardarDatosACTION(_ sender: Any) {
        if self.datosCumplimentados() {
            ...
            self.present(Utils.shared.muestraAlerta(titulo: "Saludos a todos",
                                                    mensaje: "Estamos slavando los datos con éxito",
    ...
    private func configuracionImageView() {
        ...
        let tapGR = UITapGestureRecognizer(target: self, action: #selector(muestaSelectorFoto))
        self.imagenPerfil.addGestureRecognizer(tapGR)
    }
    
    @objc
    func muestaSelectorFoto() {
    ...
    private func mostrarDatosActualizados() {
        ...
        self.fechaActualizacionLBL.text = "Ultima actualización: \(dateFormatter.string(from: Utils.Constants().kPrefs.object(forKey: Utils.Constants().kFechaActualizacion) as! Date))"
